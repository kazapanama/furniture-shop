...
import { deleteOne, updateOne } from '../../../store/ProducsReducer';
...
const AdminProduct: FC<AdminProductProps> = ({ item, type }) => {
  ...
  const handleDelete = async (id: string, images: string[]) => {
    if (confirm('Видалити продукт?(Повернути буде неможливо)')) {
  ...
  return (
    <div
      className={
        type === 'hidden' ? 'bg-gray-300 mb-1 hr py-2' : 'bg-white mb-1 hr py-2'
      }
    >
      ...
      <div className="flex justify-center gap-3 pb-2">
        <button onClick={handleShow}>
          <img
            src={item.display ? '/icons/show.svg' : '/icons/hidden.svg'}
            alt="show button"
            className="w-8 h-8 p-2 bg-green-500"
          />
        </button>
        ...
        <Link to={'/admin/product/' + item.id}>
          <img
            src="/icons/edit.svg"
            alt="edit button"
            className="w-8 h-8 p-2 bg-cyan-500 "
          />
        </Link>
